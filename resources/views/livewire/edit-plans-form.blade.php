                            ...
                            @foreach($plan['existing_planFiles'] as $existingFileIndex => $existingFile)
                                <div>
                                    <a href="{{ Storage::url($existingFile['path']) }}"
                                       target="_blank">{{ $existingFile['file_name'] }}</a>
            ...
            <div class="mt-4">
                <div>
                    <button type="button" wire:click="useTemplatePackingItems('domestic')"
                            class="border border-black bg-slate-200" @if ($template_type === 'domestic') disabled @endif>テンプレートを使う(国内版)
                    </button>
                    <button type="button" wire:click="useTemplatePackingItems('overseas')"
                            class="border border-black bg-slate-200" @if ($template_type === 'overseas') disabled @endif>テンプレートを使う(海外版)
                    </button>
                    ...
                    <div class="packing_list">
                        ...
                        <div class="mt-4">
                            <button type="button" wire:click="addPackingItem({{ $packingIndex }})"
                                    class="border border-black bg-slate-200">持ち物を追加
                            </button>
                            <button type="button" wire:click="removePackingItems({{ $packingIndex }})"
                                    class="border border-black bg-slate-200">持ち物を削除
                            </button>
                        </div>
                    </div>
                @endforeach
