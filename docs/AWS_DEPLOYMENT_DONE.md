...
| Elastic IP | `<ELASTIC_IP>`（EC2に自動アタッチ） |
...
```bash
ssh -i ~/.ssh/guide-ec2.pem ubuntu@<ELASTIC_IP>
```
...
APP_DEBUG=false
APP_URL=http://<ELASTIC_IP>  # ドメイン設定後はhttps://ドメイン名に変更
...
| 項目 | 値 |
|------|---|
| サイトURL | `http://<ELASTIC_IP>` |
| SSH接続 | `ssh -i ~/.ssh/guide-ec2.pem ubuntu@<ELASTIC_IP>` |
| SSH秘密鍵 | `~/.ssh/guide-ec2.pem` |
| SSMパラメータ名 | `<SSM_KEY_PARAMETER_NAME>` |
...
- [ ] `scripts/backup-postgres.sh` を毎日実行するcronを設定する
